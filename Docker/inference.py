...
from model.model_madmil import MADMIL
import pandas as pd
import numpy as np
...
def remap_prob(p, t_star):
    logit = lambda x: np.log(x) - np.log1p(-x)
    return 1 / (1 + np.exp(-(logit(p) - logit(t_star))))

def load_fold_models(device, n_classes=2, feat_type="uni"):
    models = []
    for k in range(5):
        ckpt_path = f"model/fold_{k}.pt"
        model = MADMIL(n_classes=n_classes, feat_type=feat_type, n=2).to(device)
...
def ensemble_predict(models, data, coords, clinic, threshold=None):
    """
    models: list of trained models
    data, coords, clinic: inputs for one slide
    threshold: global opt threshold or None
    """
    probs = []
    with torch.no_grad():
        for i, model in enumerate(models):
            _, Y_prob, _, _, _ = model(data, coords=coords, clinic=clinic)
            p = Y_prob[0][1].item()  # positive class prob
    ...
    if threshold is not None:
        # remap so that fold's opt threshold -> 0.5
        prob = remap_prob(prob, threshold)

    return prob  # final ensembled probability
...
def run():
    try:
        ...
        cli_path = INPUT_PATH / "chimera-clinical-data-of-bladder-cancer-patients.json"
        ...
        coord = torch.tensor(coords, dtype=torch.long, device=device)

        prediction = ensemble_predict(models, data, coord, clinic, threshold=global_threshold)
  
        with open(OUTPUT_PATH / "brs-probability.json", "w") as f:
            json.dump(round(prediction, 4), f, indent=4)
