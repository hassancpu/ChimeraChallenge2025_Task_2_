...
"""
X model with seperate attention modules.
...
class X(nn.Module):
    def __init__(self, n_classes=2, n=2, feat_type='uni'):
        ...
        self.sub_classifiers = nn.ModuleList([nn.Linear(self.step, 1) for _ in range(self.n_heads)]) 
        initialize_weights(self)
    ...
    def relocate(self):
        ...
        self.attention_net = self.attention_net.to(device)
        self.classifiers = self.classifiers.to(device)
        self.sub_classifiers = self.sub_classifiers.to(device)
    ...
    def forward(self, h, coords= None, attention_only=False):
        ...
        for nn in range(self.n_heads):
            logit= self.sub_classifiers[nn](M[:, nn, :].view(1, -1)) 
            logits.append(logit)

        M = M.view(1, self.size[1])

        # Singlehead Classification
        logits.append(self.classifiers(M))
