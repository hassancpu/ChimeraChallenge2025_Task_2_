...
# Description: This file contains the implementation of the CLAM model.
...
class CLAM_SB(nn.Module):
    def __init__(self, k_sample=8, n_classes=2, instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, feat_type='uni'):
        super().__init__()
        if feat_type == 'uni':
            size = [1024, 512, 256]
    ...
    def forward(self, h, label=None, instance_eval=False, attention_only=False, coords= None, clinic= None):
...
class CLAM_MB(CLAM_SB):
    def __init__(self, k_sample=8, n_classes=2,instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False):
