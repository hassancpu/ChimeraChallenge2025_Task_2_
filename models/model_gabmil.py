...
class GABMIL(nn.Module):
    def __init__(self, use_local= True, use_grid= False, use_block= False, n_classes=2, win_size_b= 1, win_size_g= 1, magnification= 10, feat_type='uni'):
        super().__init__()
        if feat_type == 'uni':
            size = [1024, 512, 256]
    ...
    def forward(self, h, coords= None, clinic= None, attention_only=False):
        ...
        #coords = torch.randint(0, 100, (h.shape[0], 2), device=device) 

        "input data with preserved spatial information"
        scale = 40//self.magnification
        ref_patch_size = 224*scale
        ...
        return logits, Y_prob, Y_hat, A_raw, _